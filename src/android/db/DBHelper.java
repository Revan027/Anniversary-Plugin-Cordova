...
public class DBHelper extends SQLiteOpenHelper {
	...
	/***************** création des table si elles n'existent pas **********************/
      ...
      public void onCreate(SQLiteDatabase db) {
            ...
            db.execSQL("create table "+TABLE_NAME_2+"("+COLUMN_ID_2+"  integer primary key AUTOINCREMENT, "+COLUMN_DATE+" text, "+COLUMN_DATE_RAPPEL+" VARCHAR(400))");
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
      ...
      public ArrayList<String> GetAll() {	
            ...
            Cursor cursor = db.rawQuery("SELECT anniversaire.id,anniversaire.name,anniversaire.num_tel,anniversaire.etat,anniversaire.id_message,anniversaire.id_date,Date.date,Date.date_rappel from "+TABLE_NAME+" INNER JOIN "+TABLE_NAME_2+" on "+TABLE_NAME+".id_date = "+TABLE_NAME_2+".id ", null);
            cursor.moveToFirst();
            
            while (!cursor.isAfterLast()) {
                  
            array_list.add(Integer.toString(cursor.getInt(0)));
                  array_list.add(cursor.getString(1));
                  array_list.add(cursor.getString(2));
                  array_list.add(Integer.toString(cursor.getInt(3)));
                  array_list.add(Integer.toString(cursor.getInt(4)));
                  array_list.add(Integer.toString(cursor.getInt(5)));
                  array_list.add(cursor.getString(6));
                  array_list.add(cursor.getString(7));
                  cursor.moveToNext();
            }
            cursor.close();
            return array_list;
